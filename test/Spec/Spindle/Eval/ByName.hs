...
module Spec.Spindle.Eval.ByName (byNameTests) where

import Spindle.Eval.ByName
import Test.Tasty
...
import Spec.Spindle.Eval.Common
import Control.Monad.IO.Class (MonadIO(..))
import System.Timeout (timeout)

byNameTests :: TestTree
byNameTests = testGroup "ByName evaluator Tests"
...
-- They use the ByName evaluator

boolExprTests :: TestTree
boolExprTests = testGroup "boolean expression tests (ByName)"
...
evalTestCase name input expected =
  testCase name $ do
    parseResult <- either (fail . show) pure $ myParse myParser name input
    case (expected, eval parseResult) of
      (Right expectedThunk, Right (evalResult :: Thunk)) -> testThunkEq expectedThunk evalResult
      (_, evalResult) -> expected @=? evalResult

simpleConstructorTests :: TestTree
simpleConstructorTests = testGroup "expr constructor tests (ByName)"
...
precedenceTests = testGroup "precedence tests (ByName)"
...
evalCondTests = testGroup "conditional tests (ByName)"
...
lamAppTests = testGroup "lambda application tests (ByName)"
  ...
  , evalTestCase "lambda application with excess parameters" "(\\ x y => x + y) # 2" (Right (NClosure (fromList [("x", NClosure mempty [] (ILit 2))]) ["y"] (BiOp (ArithOp Add) (Var "x") (Var "y"))))
...
letTests :: TestTree
letTests = testGroup "let binding tests (ByName)"
...
omegaTests :: TestTree
omegaTests = testGroup "test differences between byName and byValue (ByName)"
  ...
  , evalTestCase
    "'(\\ x => 1) # omega' evaluates to 1 in call by name"
    "let z := (\\y => y # y) in let omega := z # z in (\\ x => 1) # omega"
    (Right (NILit 1))
  ]